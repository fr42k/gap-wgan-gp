...
netGs = [Generator() for i in range(N)]
netDs = [Discriminator() for j in range(N)]

use_cuda = torch.cuda.is_available()
if use_cuda:
    gpu = 0
if use_cuda:
    netDs = [netDs[i].cuda(gpu) for i in range(N)]
    netGs = [netGs[i].cuda(gpu) for i in range(N)]
...
optimizerDs = [optim.Adam(netDs[i].parameters(), lr=1e-4, betas=(0.5, 0.9)) for i in range(N)]
optimizerGs = [optim.Adam(netGs[i].parameters(), lr=1e-4, betas=(0.5, 0.9)) for i in range(N)]
...
for iteration in range(ITERS):
    ...
    ###########################
    epoch_data = []
    s_D_cost = torch.FloatTensor([sys.maxsize])
    s_G_cost = torch.FloatTensor([sys.maxsize])
    s_D_real = torch.FloatTensor([sys.maxsize])
    s_D_fake = torch.FloatTensor([sys.maxsize])
    if use_cuda:
    ...
    lib.plot.plot('./tmp/cifar10/{}/train disc cost'.format(EXP), s_D_cost.cpu().data.numpy())
    lib.plot.plot('./tmp/cifar10/{}/time'.format(EXP), time.time() - start_time)
    lib.plot.plot('./tmp/cifar10/{}/train gen cost'.format(EXP), s_G_cost.cpu().data.numpy())
